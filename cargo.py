...
print ("Groundspeed: %s" % vehicle.groundspeed, 'm/s')
print ("Airspeed: %s" % vehicle.airspeed, 'm/s')
...
def adds_locations(aLocation): #target locations
    ...
    point1 = LocationGlobalRelative(-6.978988, 107.567913, 10)
    point2 = LocationGlobalRelative(-6.979911, 107.567264, 10)
    point3 = LocationGlobalRelative(-6.982335, 107.571390, 10)
    ...
    print("Upload New Commands To Pidrone")
    cmds.upload()
...
def arm_and_takeoff(aTargetAltitude):
    ...
    print("Basic Pre-Arm Checks ... DON'T TOUCH Pidrone !!!")
    # Don't let the user try to arm until autopilot is ready
    while not vehicle.is_armable:
        print("Waiting For Pidrone To INITIALIZE...")
        time.sleep(1)
    ...
    while True:
        print ('Altitude:', vehicle.location.global_relative_frame.alt, 'meter')      
        time.sleep(1)
...
while True:
    nextwaypoint=vehicle.commands.next
    print('Distance to waypoint %s : %s meter, Altitude: %s, meter' % (nextwaypoint, distance_to_current_waypoint(), vehicle.location.global_relative_frame.alt),)
    time.sleep(1)
    if vehicle.commands.next==3: #dummy waypoint for landing near waypoint 2
        print('Land And Release Package')
        break
...
while True:
    print('Altitude: %s' % vehicle.location.global_relative_frame.alt, 'meter')
    time.sleep(1)
    if vehicle.location.global_relative_frame.alt <= 1:
        vehicle.mode = VehicleMode("ALT_HOLD")
        print("Pidrone Landed")
        print("Do Release Package")
        break
...
print("Release Package Now")
print("Wait 5s Before Pidrone Take Off")
...
for i in range(3, -1, -1): #counting down hover time before do mission
    time.sleep(1) #3sec
    print('Count To Go Home: '+ str(i))

print('Go Home Now')

print('Pidrone Return To Home')
print("Switch Mode To RTL")
vehicle.mode = VehicleMode("SMART_RTL") #default altitude rtl 15m
while True:
    vehicle.location.global_relative_frame.alt != 0.5
    print("Return To Home, Altitude: %s" % vehicle.location.global_relative_frame.alt, 'meter')
    if vehicle.location.global_relative_frame.alt <= 0.5 : #point to vehicle.close
        print("Pidrone LANDED")
        print("DISARMING MOTORS")
        print("Pidrone Is Save For Pick Up Now")
        break
    time.sleep(1)
